...
//设置每次获取的数值
if (!isset($_COOKIE['youth_demand']))
{
    $_COOKIE['youth_demand'] = 10;
}
$random_demand = $_COOKIE['youth_demand'];
setcookie('youth_demand', $random_demand, time() + 3600 * 24 * 365);
...
try
{
    $sql = 'select count(*) from youthwords.corpus';
...
try
{
    $sql = 'select content from youthwords.corpus where id = :random_number';
    $s = $pdo->prepare($sql);
